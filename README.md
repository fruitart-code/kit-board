...
14. **Сама уезжает в Готово при закрытии.** Закрыл issue → карточка автоматически в `🏁 Готово` → через сутки `auto-archive` cron убирает с доски. Не нужно настраивать built-in *Item closed* в UI Project'а.
15. **Авто-отчёты не засоряют kanban.** Pinned issues от metrics / dependency-graph / board-audit workflow'ов автоматически архивируются на доске — остаются в Issues view для weekly updates, но не висят в `📋 К работе`. Список меток-исключений настраивается в `config.yml::reports.exclude_labels`.

Под капотом: **13 consistency rules** в декларативном `config.yml`, **6 workflows**, **20 auto-actions**. Отключается одной командой.
